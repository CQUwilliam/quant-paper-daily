...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("arxiv_qfin_crawler.log"), logging.StreamHandler()]
)
...
INITIAL_ARXIV_URL = "https://arxiv.org/list/q-fin/recent?show=100"  # q-fin领域最新论文

# 存储配置
JSON_SAVE_PATH = "arxiv_q_fin_papers_final.json"
...
def json_to_markdown(json_path: str, md_path: str) -> None:
    ...
    md_title = f"# arXiv 量化金融领域论文汇总（共{total_papers}篇）"
    md_intro = "> 说明：仅显示最近五天数据，当天论文默认展开，其他日期点击标题可展开/折叠\n"
    md_intro += "> 相关性评分：基于LLM对量化金融领域的相关性评定（1-5分，★越多相关性越高）\n\n"
...
if __name__ == "__main__":
    ...
    # 2. 初始化日志
    logging.info("="*60)
    logging.info("        arXiv q-fin 领域论文爬取与LLM总结程序        ")
    logging.info("="*60)
    logging.info(f"配置信息：")
